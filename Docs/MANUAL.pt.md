...
*Tradução. O documento principal é o [MANUAL.md](MANUAL.md), em inglês.*
...
**Isso não aconteceu em produção.** A loja está no ar com jogadores reais
comprando: sem queda, sem perda de ponto, sem saldo errado. A janela está
descrita aqui porque existe no código, não porque alguém tenha caído nela.
...
### Itens de DLC — não testados

A entrega está provada para os **itens vanilla**: todos chegam. Itens de DLC não
foram testados, e enquanto não forem, isso fica na coluna "não provado" e não na
de "funciona".

O catálogo gerado os exclui por padrão (`DLCPackage != None`) por outro motivo:
quem não tem o DLC compraria e não receberia. Se você habilitar com `--com-dlc`,
essa responsabilidade passa a ser sua — o plugin não tem como saber quais DLCs
cada jogador possui.
...
| `!comprar` entregando o item | **provado com jogador real** — todo item vanilla entrega; o primeiro foram 100 pedras no inventário |
